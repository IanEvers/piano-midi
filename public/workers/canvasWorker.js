...
var tocarNota = function(nota) {
    var i = keysPressed.length;
    while(i--) {
        if(keysPressed[i]== nota) {
            return false;	
        }
    }
    keysPressed.push(nota);
    
    if(keyboard[nota]) {
        var arrPlayNote = keyboard[nota].split(',');
        var note = arrPlayNote[0];
        var octaveModifier = arrPlayNote[1]|0;
        piano.play(note, __octave + octaveModifier, 2);
    } else {
        return false;	
    }
}

var  removerNota = function(e) {
    var i = keysPressed.length;
    while(i--) {
        if(keysPressed[i]==e) {
...
self.addEventListener('message', function(e) {
    if(e.data.play) {
        tocarNota(e.data.play)
    } else {
        removerNota(e.data.stop)    
    }
}, false);